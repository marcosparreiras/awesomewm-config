package.path = package.path .. ";" .. os.getenv("HOME") .. "/.config/awesome/vicious/?.lua;"
package.path = package.path .. ";" .. os.getenv("HOME") .. "/.config/awesome/vicious/?/init.lua;"
...
awful.widget.watch(
	...
	function(widget, stdout)
		widget:set_text("VOL: " .. stdout:gsub("\n", ""))
	end,
	volume_widget
)
...
vicious.register(memwidget, vicious.widgets.mem, function(widget, args)
	local used_gb = args[2] / 1024 -- usado em GB
	local total_gb = args[3] / 1024 -- total em GB
	return string.format(" RAM: %d%% (%.1fGB/%.1fGB) ", args[1], used_gb, total_gb)
end, 5)
...
awful.widget.calendar_popup
	...
	:attach(mytextclock, "tr", { on_hover = false })
...
awful.screen.connect_for_each_screen(function(s)
	...
	s.mywibox:setup({
		...
		{ -- Right widgets
			...
			wibox.widget.textbox(" | "),
			-- CPU
			wibox.container.margin(cpuwidget, 5, 0, 0, 0),
			wibox.widget.textbox(" | "),
			-- Memory widget
			wibox.container.margin(memwidget, 5, 0, 0, 0),
			wibox.widget.textbox(" | "),
			-- Batery
			wibox.container.margin(batwidget, 5, 0, 0, 0),
			wibox.widget.textbox(" | "),
			-- Volume
			wibox.container.margin(volume_widget, 5, 0, 0, 0),
			wibox.widget.textbox(" | "),
			-- Keyboard layout
			wibox.container.margin(mykeyboardlayout, 5, 0, 0, 0),
			wibox.widget.textbox(" | "),
			-- Clock
			wibox.container.margin(mytextclock, 5, 0, 0, 0),
			-- Systray
			wibox.container.margin(wibox.widget.systray(), 5, 0, 0, 0),
			-- Layoutbox
			-- wibox.container.margin(s.mylayoutbox, 5, 0, 0, 0),
		},
	})
end)
-- }}}
...
globalkeys = gears.table.join(
	...
	end, { description = "take a screenshot", group = "print" }),
	awful.key({ modkey }, "=", function()
		awful.spawn("pactl set-sink-volume " .. sink .. " +5%")
